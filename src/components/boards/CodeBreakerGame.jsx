...
import { generateCodeBreaker } from "../../utils/generators/codeBreakerGenerator";
...
export default function CodeBreakerGame({ puzzle }) {
  ...
  const [roundResults, setRoundResults] = useState([]); // { round, sequence: examplesAndTarget, studentAnswer, correctAnswer, isCorrect, rule }
  ...
  const loadChallenge = (diffOverride = null) => {
    ...
    const nextChallenge = getUniqueQuestion(`code-breaker-${nextDiff}`, () => generateCodeBreaker(nextDiff), (q) => q.targetWord + "-" + q.answer);
  ...
  const handleSelectOption = (opt) => {
    ...
    // Save examples + target word representation for review
    const sequenceRep = [
      ...challenge.examples.map(ex => `${ex.word} = ${ex.code}`),
  ...
  const handleNextRound = () => {
    if (currentRound < TOTAL_ROUNDS) {
      ...
      const breakScore = Math.round(accuracyPercent * diffMultiplier);
      ...
      submitAttempt(
        ...
        difficulty,
        {
          ...
          bestStreak,
          ruleDiscoveryScore: breakScore,
          reasoningScore: breakScore,
          roundResults
        }
      );
          ...
          <div className="flex-1 flex flex-col justify-between animate-[brain-fade-in-up_0.4s_ease-out]">
            <div>
              ...
              <div className="bg-white rounded-2xl border border-saathi-line p-5 shadow-card mb-6 text-center">
                <p className="text-[10px] font-extrabold uppercase text-saathi-muted tracking-wider mb-4">
                  Encoding Cipher Examples
                </p>
                
                {/* Example Pairs */}
                <div className="bg-gray-50 border border-saathi-line p-4 rounded-2xl mb-4 font-black text-lg space-y-2.5 text-center shadow-inner">
                  {challenge.examples.map((ex, idx) => (
                    <div key={idx} className="flex justify-center items-center gap-2">
                      <span className="bg-white px-3 py-1 border border-saathi-line rounded-lg text-saathi-ink text-sm shadow-sm">{ex.word}</span>
                      <span className="text-saathi-muted text-sm">=</span>
                      <span className="bg-indigo-50 text-saathi-indigo px-3 py-1 border border-indigo-100 rounded-lg text-sm shadow-sm">{ex.code}</span>
                    </div>
                  ))}
                </div>

                {/* Target word */}
                <div className="flex justify-center items-center gap-2 font-black text-xl mb-4">
                  <span className="bg-white px-4 py-2 border-2 border-saathi-indigo text-saathi-indigo rounded-xl shadow-md">{challenge.targetWord}</span>
                  <span className="text-saathi-muted">=</span>
                  <span className="bg-amber-50 text-saathi-amber px-4 py-2 border-2 border-dashed border-saathi-amber rounded-xl animate-pulse">?</span>
                </div>
              ...
              {isAnswered && (
                ...
                >
                  ...
                  <p className="text-xs text-saathi-ink font-semibold leading-relaxed mb-4 whitespace-pre-line text-left">
