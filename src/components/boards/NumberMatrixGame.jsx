...
import { generateNumberMatrix } from "../../utils/generators/numberMatrixGenerator";
...
export default function NumberMatrixGame({ puzzle }) {
  ...
  const [roundResults, setRoundResults] = useState([]); // { round, sequence: matrix, studentAnswer, correctAnswer, isCorrect, rule }
  ...
  const handleSelectOption = (opt) => {
    ...
    setRoundResults((prev) => [
      ...
      {
        round: currentRound,
        sequence: challenge.matrix, // Save displayed matrix as the sequence representation for review
  ...
  const handleNextRound = () => {
    if (currentRound < TOTAL_ROUNDS) {
      ...
      const matrixLogicScore = Math.round(accuracyPercent * diffMultiplier);
      ...
      submitAttempt(
        ...
        difficulty,
        {
          ...
          bestStreak,
          logicScore: matrixLogicScore,
          patternRecognitionScore: matrixLogicScore,
          roundResults
        }
      );
          ...
          <div className="flex-1 flex flex-col justify-between animate-[brain-fade-in-up_0.4s_ease-out]">
            <div>
              ...
              <p className="text-sm font-semibold text-saathi-muted mb-8 leading-relaxed">
                Develop strong pattern recognition and analytical logic skills. Analyze number trends across rows and columns to find the missing value in the matrix.
              </p>
          ...
          <div className="flex-1 flex flex-col justify-between animate-[brain-fade-in-up_0.4s_ease-out]">
            <div>
              ...
              <div className="bg-white rounded-2xl border border-saathi-line p-5 shadow-card mb-6 text-center">
                ...
                {/* 3x3 Matrix Grid */}
                <div className="grid grid-cols-3 gap-2 mx-auto w-48 mb-4 bg-gray-50 border border-saathi-line p-3 rounded-2xl shadow-inner">
                  {challenge.matrix.flat().map((cell, idx) => {
                    const isQuestion = cell === "?";
                    return (
                      <div key={idx} className={`aspect-square flex items-center justify-center text-xl font-black rounded-xl border shadow-sm ${
                        isQuestion 
